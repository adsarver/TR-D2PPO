...
import threading
from utils.utils import load_buffer_from_pkl, buffer_to_pkl
...
class ControlHandler:
    ...
    def __init__(self, env, current_map, params, max_demo_buffer=10000, use_gamepad=True, load_map=None):
        """
        ...
        load_map = load_map if load_map else current_map
        self.demonstration_buffer = self._load_demonstrations(load_map)
    ...
    def _load_demonstrations(self, map_name):
        ...
        try:
            file = f"demonstrations_up_to_gen_{map_name}.pkl"
            return load_buffer_from_pkl(file)
    ...
    def _handle_gamepad_event(self, event):
        ...
        """
        if event.code == 'ABS_HAT0Y' and event.state == 1:  # Down button
            self.render = not self.render
            print(f"\n## RENDERING {'ENABLED' if self.render else 'DISABLED'}\n")
    ...
    def override_action(self, scaled_values):
        """
    ...
    def store_demonstration(self, scan_tensor, state_tensor, action_tensor, just_crashed, rewards):
        """
        ...
        Args:
            ...
            rewards: Optional array of rewards for value estimation during pretraining
        """
        ...
        # Store observation, human action (normalized), and reward for value estimation
        demo_obs = {
            ...
            'value': rewards[self.controlled_agent_idx]
        }
